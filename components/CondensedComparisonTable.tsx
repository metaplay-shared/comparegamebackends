  ...
  if (result.classification === 'none') {
    return (
      <td className="py-3 px-2 text-center">
        <svg className="h-5 w-5 text-slate-300 dark:text-slate-600 mx-auto" viewBox="0 0 20 20" fill="currentColor">
          <path
            fillRule="evenodd"
    ...
    <td className="py-3 px-2 text-center">
      ...
      >
        ...
        {isVisible && result.supportedFeatures.length > 0 && position && (
          <div
            className="fixed z-[9999] w-[200px] p-3 bg-slate-900 dark:bg-slate-800 text-white text-xs rounded-lg shadow-lg border border-slate-700"
            style={{
          ...
          >
            ...
            />
            <div className="font-medium mb-1.5 text-slate-200">
              {result.supportedFeatures.length} of {result.totalFeatures} features:
            </div>
            <ul className="space-y-0.5">
              {result.supportedFeatures.map((feature) => (
                <li key={feature} className="flex items-center gap-1.5 text-slate-200">
                  <svg className="h-3 w-3 text-green-400 shrink-0" viewBox="0 0 20 20" fill="currentColor">
                    <path
                      fillRule="evenodd"
        ...
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-700">
            ...
            {categories.map((category) => (
              <th key={category} className="text-center py-3 px-2 font-semibold whitespace-nowrap">
